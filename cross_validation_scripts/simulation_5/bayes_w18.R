...
max_dp = max(input_data$AP)
  ...
  input_data<-input_data[,.SD,.SDcols = c("claim_number", "business_use", "property_value" , "age" , "AP_o", "DP_o")]
...
dt_target[, (common_cols):=dt_covariates[,..common_cols]]    
  ...
  name = paste0("~/modi_mount/ReSurv_cv_results",  "/sim",scenario_number,"_",i,"_marios_cv_",format(Sys.time(), "%Y_%m_%d_%H:%M"),".csv")

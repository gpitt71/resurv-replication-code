...
for (i in seeds) {
  ...
  setDT(input_data)
  input_data <- input_data[sample(.N)]

  input_data <- input_data[1:ceiling(nrow(input_data) * .4), ]
  ...
  bayes_out <- bayesOpt(
    FUN = obj_func,
    bounds = bounds,
    initPoints = 8, # >= number of hyperparameters
    iters.n = 20,
    iters.k = 4,
    otherHalting = list(timeLimit = 1800),
    parallel = TRUE
  )
  ...
  name <- file.path(
    cv_results_dir,
    sprintf("sim5_%s_deepsurv_cv_%s.csv", i, format(Sys.time(), "%Y_%m_%d_%H:%M"))
  )
